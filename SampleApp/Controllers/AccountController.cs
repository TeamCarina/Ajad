...
namespace SampleApp.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public AccountController(ApplicationUserManager userManager, ApplicationSignInManager signInManager,IAccountLogic _accountLogic,IAdminLogic _adminLogic)
        {
        ...
        public List<string> GetCountryList()
        {
            ...
            foreach (var getCulture in getCultureInfo)
            {
                ...
                if (!result.Contains(getregionInfo.EnglishName))
                {
                    result.Add(getregionInfo.EnglishName);
                }
            }
            result.Sort();
            return result;
        ...
        public async Task<ActionResult> Register([Bind(Exclude = "AppId,ConfirmPassword,Country,Address,City,Landmark,PhoneNumber")] RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
              //  AppModel model2;
              //model2= await  CreateApp(model.FirstName,model.Subdomain);
                ...
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email, FirstName = model.FirstName, LastName = model.LastName, Website = model.Website, SubDomain = model.Subdomain, AppId = AppId };
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                // For more information on how to enable account confirmation and password  please visit http://go.microsoft.com/fwlink/?LinkID=320771
                // Send an email with this link
        ...
        public async Task<ActionResult> ExternalLoginConfirmation(ExternalLoginConfirmationViewModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email, PhoneNumber = model.PhoneNumber, Country = model.Country, FirstName = model.FirstName, LastName = model.LastName, Landmark = model.Landmark, City = model.City, Address = model.Address };
                var result = await UserManager.CreateAsync(user);
                if (result.Succeeded)
                {
                    UserManager.AddToRole(user.Id, "Customer");
                    accountLogic.OpenAccount(user.Id);
